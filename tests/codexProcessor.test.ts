...
describe("CodexProcessor", () => {
	...
	const makeStateManager = (issueNumber: number) => {
		...
		const store = new Map<string, IssueState>([
			[key(issueNumber, "codex"), state],
		]);
		return {
			branch,
			getState: mock((issue: number, processor: string) =>
				store.get(key(issue, processor)),
			),
	...
	test("runs codex CLI with expected arguments and completes", async () => {
		...
		const commands = runCommandMock.mock.calls.map(([cmd]) =>
			(cmd as string[]).join(" "),
		);
