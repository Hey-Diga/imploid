...
export class CodexProcessor {
	...
	async processIssue(
		...
		const branchName =
			...
			createIssueBranchName(issueNumber, PROCESSOR_NAME);
		...
		const { process, stdout, stderr } = spawnProcess(codexArgs, {
			cwd: repoPath,
		});
		...
		while (exitCode === null) {
			const exited = await Promise.race([
				...
				new Promise<number | null>((resolve) =>
					setTimeout(() => resolve(null), checkInterval),
				),
			]);
